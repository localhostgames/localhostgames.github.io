...
class Moon {
    constructor(name, gravity, surfaceColor, terrainData) {
        this.name = name;
        this.gravity = gravity;
        this.surfaceColor = surfaceColor;
        this.terrainData = terrainData; // Could hold the Landscape data
		this.moonMult = 0;
    }
}
...
const moons = [
    new Moon("Default Moon", 1.62, "gray", generateTerrain(), 0),
    new Moon("Red Moon", 1.5, "red", generateTerrain(), 2),
    new Moon("Blue Moon", 1.8, "blue", generateTerrain(), 5),
];
let currentMoonIndex = 0;

function generateTerrain() {
    // Generate procedural terrain based on specific moon properties
    return new Landscape(); // Customize landscape generation logic
}

function loadMoon(moon) {
    console.log(`Switching to ${moon.name}`);
    landscape = moon.terrainData; // Load the new terrain
    lander.gravity = moon.gravity; // Adjust gravity
    restartLevel(false); // Reset lander and game state
}
...
//				console.log(map(touchController.leftTouch.getX(), SCREEN_WIDTH*touchRotateLeft, SCREEN_WIDTH*touchRotateRight, -90,90,true));
	...
	if (lander.altitude >= 1000) {
		currentMoonIndex = (currentMoonIndex + 1) % moons.length;
		loadMoon(moons[currentMoonIndex]);
	}
...
function render() { 
	...
	landscape.render(context, view, moons[currentMoonIndex].surfaceColor);
...
function restartLevel(resetFuel) { 
	lander.reset(resetFuel); 
	landscape.setZones(moons[currentMoonIndex].moonMult);  // moon zone multiplier (defaultzonemult + moonmult)
	setZoom(false); 
